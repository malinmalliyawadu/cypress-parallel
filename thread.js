...
function createReporterConfigFile(path) {
  const reporterEnabled = ['@malinmalliyawadu/cypress-parallel/json-stream.reporter.js'];
  let reporterName = settings.reporter;
  if (settings.reporter) {
    reporterEnabled.push(reporterName);
  } else {
    reporterEnabled.push('@malinmalliyawadu/cypress-parallel/simple-spec.reporter.js');
  }
  const content = {
